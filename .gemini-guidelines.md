...
-   **Atomicité** : Chaque commit doit être atomique, ne traitant qu'une seule tâche, fonctionnalité ou correction de bug à la fois. Si une modification implique plusieurs changements distincts, ils doivent être divisés en plusieurs commits.
...
-   **Priorité 3 : Commits Atomiques**
    -   **Règle** : Effectuer des commits atomiques après chaque modification significative, chaque correction de bug ou chaque étape du cycle TDD (Red, Green, Refactor).
    -   **Bénéfice** : Facilite la lecture de l'historique, le débogage (git bisect) et le retour en arrière si nécessaire.